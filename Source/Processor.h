...
class RadioBoxProcessor final:public juce::AudioProcessor,private juce::AudioProcessorValueTreeState::Listener {
 std::array<std::atomic<float>*,16> raw{};std::atomic<unsigned> revision{0};std::atomic<bool> restoring{false};std::atomic<int> activeProgram{0};havi::Controls audioControls=havi::factoryControls[0];unsigned audioRevision=~0u;int audioProgram=0;
 ...
 void applyFactory(int index){revision.fetch_add(1,std::memory_order_acq_rel);restoring.store(true);activeProgram.store(index);const auto& c=havi::factoryControls[size_t(index)];for(size_t i=0;i<c.size();++i)setRaw(havi::controlIDs[i],c[i]);restoring.store(false);revision.fetch_add(1,std::memory_order_release);}
public:
 ...
 static juce::AudioProcessorValueTreeState::ParameterLayout layout(){juce::AudioProcessorValueTreeState::ParameterLayout p;juce::StringArray names;for(auto* n:havi::presetNames)names.add(n);p.add(std::make_unique<juce::AudioParameterChoice>(juce::ParameterID{"preset",1},"Preset",names,0));
  ...
  p.add(std::make_unique<juce::AudioParameterBool>(juce::ParameterID{"vuLight",1},"VU Backlight",true));p.add(std::make_unique<juce::AudioParameterChoice>(juce::ParameterID{"theme",1},"UI Color",juce::StringArray{"Vintage Green","Burgundy","Ice Blue"},0));return p;}
 RadioBoxProcessor():AudioProcessor(BusesProperties().withInput("Input",juce::AudioChannelSet::stereo(),true).withOutput("Output",juce::AudioChannelSet::stereo(),true)),state(*this,nullptr,"HaviRadio",layout()){for(size_t i=0;i<raw.size();++i)raw[i]=state.getRawParameterValue(havi::controlIDs[i]);state.addParameterListener("preset",this);}
 ~RadioBoxProcessor()override{state.removeParameterListener("preset",this);}havi::Controls controls()const{havi::Controls c{};for(size_t i=0;i<c.size();++i)c[i]=raw[i]->load();return c;}
 ...
 void setBandEnabled(int band,bool enabled){band=juce::jlimit(0,2,band);auto* p=state.getParameter(havi::controlIDs[size_t(11+band)]);p->beginChangeGesture();p->setValueNotifyingHost(enabled?1.f:0.f);p->endChangeGesture();}
 ...
 void loadUserState(const juce::ValueTree& tree){if(!tree.isValid()||tree.getType()!=state.state.getType())return;restoring.store(true);revision.fetch_add(1,std::memory_order_acq_rel);state.replaceState(tree.createCopy());activeProgram.store(juce::jlimit(0,9,int(state.getRawParameterValue("preset")->load())));revision.fetch_add(1,std::memory_order_release);restoring.store(false);}
 ...
 void processBlock(juce::AudioBuffer<float>& b,juce::MidiBuffer&)override{juce::ScopedNoDenormals guard;float peak=0;for(int c=0;c<b.getNumChannels();++c)for(int i=0;i<b.getNumSamples();++i){const float x=b.getSample(c,i);if(std::isfinite(x))peak=std::max(peak,std::abs(x));}meter.push(peak);overload.store(peak>=.988f);
  ...
  const auto& v=audioControls;engine.process(b.getArrayOfWritePointers(),b.getNumChannels(),b.getNumSamples(),v[0],v[1],v[2],v[3],v[4],v[5],v[6],v[7],v[8],v[9],v[10],v[11]>.5f,v[12]>.5f,v[13]>.5f,v[14]>.5f,v[15]>.5f);
 }
