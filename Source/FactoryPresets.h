...
inline bool matchesFactory(const Controls& c,int index){const auto& e=factoryControls[size_t(std::clamp(index,0,9))];for(size_t i=0;i<c.size();++i)if(std::abs(c[i]-e[i])>.005f)return false;return true;}
