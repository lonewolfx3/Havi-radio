...
public:
 ...
 void process(float* const* data,int channels,int samples,float lo,float midDb,float hi,float bandwidth,float tuning,float filter,float drive,float tone,float width,float wet,float outDb,int band,bool dropout,bool power){
  if(!power)return;const float tune=std::pow(2.f,std::clamp(tuning,-80.f,100.f)/240.f),bandWidth=.35f+std::clamp(bandwidth,0.f,100.f)*.012f;
  ...
  high*=.35f+.65f*std::clamp(filter,0.f,100.f)/100.f;high*=std::pow(2.f,std::clamp(tone,-1.f,1.f));if(band==0){low*=1.35f;high*=.62f;}else if(band==2)high*=.78f;
  ...
  for(int i=0;i<samples;++i){phase+=6.28318530718*(band==0?2.3:band==1?5.2:8.5)/sampleRate;if(phase>6.28318530718)phase-=6.28318530718;const float targetDrop=dropout&&randomUnit()<.00022f?(.10f+.22f*randomUnit()):1.f;dropoutGain+=(targetDrop-dropoutGain)*(targetDrop<dropoutGain?.15f:.0025f);float dry[2]{0,0},fx[2]{0,0};
   for(int c=0;c<std::min(channels,2);++c){dry[c]=std::isfinite(data[c][i])?std::clamp(data[c][i],-8.f,8.f):0.f;auto& s=state[size_t(c)];s.hp+=hpA*(dry[c]-s.hp);float y=dry[c]-s.hp;s.midLp+=midA*(y-s.midLp);y+=s.midLp*midGain;y=std::tanh(y*saturation)/std::sqrt(saturation);if(dropout)y+=(randomUnit()*2-1)*.012f;s.lp+=lpA*(y-s.lp);fx[c]=s.lp*dropoutGain*(1-.025f*std::sin(phase));}
