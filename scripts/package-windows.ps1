...
$plugin = [IO.Path]::GetFullPath('build-windows/HaviRadio_artefacts/Release/VST3/Havi Radio.vst3')
if (-not (Test-Path "$plugin/Contents/x86_64-win/Havi Radio.vst3")) {
...
$output = [IO.Path]::GetFullPath('dist/HaviRadio-Windows-x64-Setup.exe')
